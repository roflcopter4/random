...
void quick_sort(uint16_t *data, uint16_t size);
...
static inline uint16_t get_val(int argc, char **argv);
...
main (int argc, char **argv)
{
        ...
        uint16_t tokens = get_val(argc, argv);
...
get_val(int argc, char **argv)
{
        extern int optind;
        if (argc == optind)
                do { fprintf(stderr, "Error: No value entered.\n"); exit(5); } while (0);
        return (uint16_t)xatoi(argv[optind]);
}
