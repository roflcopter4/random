...
class Grid():
    def __init__(self, row=4, col=4, initial=2):
        ...
        self.__hbar = '\t|%s\n' % (col*'-----|')
        self.len = row * col
        self.emptiesSet = list(range(self.len))  # list of empty cells
        self.new_cell = None
...
#
    ...
    def collapseUp(self):
        """Creates a list for each collumn in the grid and runs it through
        ...
        """
        ret = False
        for rInd in range(0, self.row):
            lst = self.get_columns(rInd)
            lst, tmp = self.collapseRow(lst)
            ret = ret or tmp
            x = 0
            for n in range(0, self.len, self.col):
                self.set_cell(rInd+n, lst[x])
                x += 1
        return ret

    def collapseDown(self):
        """Creates a list for each column in the grid, reverses it, runs it
        through collapseRow, and reverses it again. The changed values are used
        to set the values in the main grid. Returns True if any columns
        collapsed downards.
        """
        ret = False
        for rInd in range(0, self.row):
    ...
    def get_columns(self, rInd):
        return [self.get_cell(rInd+n) for n in range(0, self.len, self.col)]
