...
#
    ...
    def collapseUp(self):
        ...
        for rInd in range(0, self.row):
            lst = [self.get_cell(rInd+n) for n in range(0, self.len, self.col)]
...
class Game():
    ...
    def play(self):
        ...
        # while (not stop) and (collapsible) and (not self.grid.WIN):
        while (not stop) and (collapsible):
            ...
            for i in range(self.grid.len):
                if int(self.grid.get_cell(i)) == 2048:
                    self.grid.WIN = True
