...
int main(void)
{
    ...
    while (true)
    {
        char cur_player = players[cur_player_ind];
        int move;
        ...
        if ((move = get_move(board, cur_player)) > 0)
        {
            ...
            if (ply >= 5)
            {
                ...
                {
                    snprintf(msg, STRSIZE, "Congratulations! Player '%c' has won!", cur_player);
                    break;
                }
