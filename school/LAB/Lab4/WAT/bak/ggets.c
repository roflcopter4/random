...
/* fggets and ggets [which is fggets(line, stdin)] set *line to
   ...
   errors some positive value is returned.  In this case *line
   may point to a partial line.  For other errors memory is
   freed and *line is set to NULL.
...
int fggets(char **line, FILE *file)
{
    int     cursize, ch, ix;
    char   *buffer, *temp;
    ...
    *line = NULL; /* default */
    ...
    if (NULL == (temp = realloc(buffer, (size_t)ix + 1)))
        *line = buffer; /* without reducing it */
    else
        *line = temp;
    return OK;
} /* fggets */
