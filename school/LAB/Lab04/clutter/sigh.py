    ...
    def make_drawing(self):
        ...
        for x1 in range(0, size[0]):
            ...
            for y1 in range(0, size[1]):
                ...
                temp1.append(walls[NORTH])
                temp2.append(walls[SOUTH])
                temp3.append(walls[EAST])
                temp4.append(walls[WEST])
            data[NORTH].append(temp1)
            data[SOUTH].append(temp2)
            data[EAST].append(temp3)
            data[WEST].append(temp4)
        ...
        for iter1 in range(0, len(Y_All)):
            for basteen in Y_All[iter1]:
               sys.stdout.write(basteen)
            print()

        for iter2 in range(0, len(X_All)):
            for basteen in X_All[iter2]:
               sys.stdout.write(basteen)
            print()
