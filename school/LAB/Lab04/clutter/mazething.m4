...
SOUTH = 2
WEST = 3
dnl define(`NORTH', 0)dnl
dnl define(`EAST', 1)dnl
dnl define(`SOUTH', 2)dnl
dnl define(`WEST', 3)dnl
X_STR = "|  "
Y_STR = `"'BOLD(---)`"'
X_PAD = "   "
Y_PAD = "   "
dnl define(`X_STR', ``"│  "'')dnl
dnl define(`Y_STR', `"'BOLD(---)`"')dnl
dnl define(`X_PAD', ``"   "'')dnl
dnl define(`Y_PAD', ``"   "'')dnl
...
class Maze:
    """Constructs and represents a whole maze, given a fi`len'ame."""
...
def solve(file_name):
    ...
    print("`'BOLD(-)"*60)
    print("YELLOW(Attempting to solve maze in file: ')`'MAGENTA(%s)`'YELLOW('.)\n" % file_name)
    maze.write_drawing()
    ...
    if is_solvable:
        print("bGREEN(Maze is solvable.)")
    else:
        print("bRED(Maze cannot be solved.)")
