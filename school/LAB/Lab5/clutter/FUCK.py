...
def solve(expr):
    ...
    while len(operands) > 1:
        # if len(optypes) == 0:
            # print("EXPR - > %s\nOPS -> %s" % (expr.operands, expr.optypes))
            # print(dumblist)
            # x = len(operands) - 1
            # optypes = dumblist[-x:]
            # print(optypes)
        maxbind = max([prec[i] for i in [dumblist[m] for m in range(0, len(operands) - 1)]])
        for x in range(0, len(operands) - 1):
            op = dumblist[x]
            if prec[op] == maxbind:
                # print(len(optypes), len(operands))
                a = operands.pop(x)
                b = operands.pop(x)
                # optypes.pop(x)
                dumblist.pop(x)
