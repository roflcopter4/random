...
extern int _IO_vfscanf(_IO_FILE *__restrict, const char *__restrict, __gnuc_va_list,
                       int *__restrict);
...
extern char *tempnam(const char *__dir, const char *__pfx) __attribute__((__nothrow__))
...
extern FILE *freopen(const char *__restrict __filename, const char *__restrict __modes,
...
extern size_t fread_unlocked(void *__restrict __ptr, size_t __size, size_t __n,
                             FILE *__restrict __stream);
extern size_t fwrite_unlocked(const void *__restrict __ptr, size_t __size, size_t __n,
                              FILE *__restrict __stream);
...
enum operators { None = 0, Add = '+', Subtract = '-', Multiply = '*', Divide = '/', Modulo = '%' };
...
struct expression {
        struct calc_op ops[32];
        enum operators operator[32];
        double ans;
        int numops;
};
...
extern const unsigned short int **__ctype_b_loc(void) __attribute__((__nothrow__))
__attribute__((__const__));
extern const __int32_t **__ctype_tolower_loc(void) __attribute__((__nothrow__))
__attribute__((__const__));
extern const __int32_t **__ctype_toupper_loc(void) __attribute__((__nothrow__))
__attribute__((__const__));
...
extern double atof(const char *__nptr) __attribute__((__nothrow__)) __attribute__((__pure__))
__attribute__((__nonnull__(1)));

extern int atoi(const char *__nptr) __attribute__((__nothrow__)) __attribute__((__pure__))
__attribute__((__nonnull__(1)));

extern long int atol(const char *__nptr) __attribute__((__nothrow__)) __attribute__((__pure__))
__attribute__((__nonnull__(1)));


__extension__ extern long long int atoll(const char *__nptr) __attribute__((__nothrow__))
...
extern long int a64l(const char *__s) __attribute__((__nothrow__)) __attribute__((__pure__))
__attribute__((__nonnull__(1)));
...
extern void *aligned_alloc(size_t __alignment, size_t __size) __attribute__((__nothrow__))
__attribute__((__malloc__));
...
__extension__ extern long long int llabs(long long int __x) __attribute__((__nothrow__))
__attribute__((__const__));


extern div_t div(int __numer, int __denom) __attribute__((__nothrow__)) __attribute__((__const__));
extern ldiv_t ldiv(long int __numer, long int __denom) __attribute__((__nothrow__))
__attribute__((__const__));
...
inline static void assign_operator2(struct expression *expr, int ch, int paren);
inline static void assign_buffer2(struct expression *expr, char *buf, int *buf_index, int *token);


int main(void)
{
        StringArray data = {((void *)0), 0};

        if (get_all_lines(&data, "NOPE.txt") != 0)
                exit(1);
...
static struct expression parse_line(String *line)
{
                ...
                if (((*__ctype_b_loc())[(int)((ch))] & (unsigned short int)_ISdigit)) {
                        ...
                        case '(':
                                if (dash) {
                                        do {
                                                if (paren &&
                                                    (expr.ops[expr.numops].operator== None))
                                                        expr.ops[expr.numops].operator=('-');
                                                else
                                                        expr.operator[expr.numops++] =('-');
                                        } while (0);
                        ...
                        case ' ':
                                if (dash) {
                                        ...
                                        do {
                                                expr.ops[expr.numops]
                                                    .num[expr.ops[expr.numops].ind++] = atoi(buf);
                                                token = buf_index = 0;
                                                buf[buf_index] = '\0';
                                        } while (0);
                                }
                        ...
                        default:
                                ...
                                do {
                                        if (paren && (expr.ops[expr.numops].operator== None))
                                                expr.ops[expr.numops].operator=(ch);
                                        else
        ...
        if (token)
                expr.ops[expr.numops].num[expr.ops[expr.numops].ind++] = atoi(buf);
...
static void print_results(struct expression *expr)
{
        ...
        for (int i = 0; i <= expr->numops; ++i) {
                if (expr->ops[i].operator!= None) {
                        sprintf(buf, "(%d %d %c) ", expr->ops[i].num[0], expr->ops[i].num[1],
                                expr->ops[i].operator);
                        expr->ops[i].ans =
                            do_operation((expr->ops[i].operator), (double)(expr->ops[i].num[0]),
                                         (double)(expr->ops[i].num[1]));

                        my_string_app(&msg, buf);
                } else {
                        sprintf(buf, "%d ", expr->ops[i].num[0]);
                        expr->ops[i].ans = (double)expr->ops[i].num[0];
                        my_string_app(&msg, buf);
                }
                if (i > 0) {
                        ...
                        if (i == 1)
                                expr->ans = do_operation(expr->operator[i - 1],
                                                         expr->ops[i - 1].ans, expr->ops[i].ans);
                        else if (i > 1)
                                expr->ans = do_operation(expr->operator[i - 1], expr->ans,
                                                         expr->ops[i].ans);
                }
        }
