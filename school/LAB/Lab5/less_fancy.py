    ...
    def format_optype(self, index, n):
        ...
        if buf is not None:
            if (self.color_op is not None) and (self.color_op == n):
                buf = "\033[1m\033[32m%s\033[0m " % buf
            else:
                buf += " "
        return buf
...
def parse_line(line, in_paren=False):
    ...
    function for each parenthetical sub-expression. 'dash' stores whether the
    previous character was a '-' in order to resolve the ambiguity between
    negation and subtraction. 'token' stores whether the previous character(s)
    were numbers or sub-expressions rather than operators or ignored characters.
    'buf' holds the current token, including any minus signs, and is converted
    to an operand when the token ends. 'paren' holds the current level of
    parentheses - calculations continue only when it is zero.
    """
    dash = token = False
    ...
    while len(operands) > 1:
        ...
        for x in range(0, len(optypes)):
            ...
            if precidence[op] == maxprec:
                ...
                # This should be one step but pyflakes whines about 'a' and 'b'
                # being unused unless I break it into two steps like this.
                a = operands.pop(x)
                b = operands.pop(x)
                a = float(get_val(a))
                b = float(get_val(b))
