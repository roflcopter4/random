...
sub html($op) {
    if ( $op =~ /init/i ) { $output = "<html>\n\n" }
    elsif ( $op =~ /end|finish/i ) { $output .= "</html>\n" }
    else                           { carp 'Invalid operation.' }
}
        ...
        add 'tr';
            ...
            foreach my $section (@sectioned_data) {
                ...
                css 'td', 'valign="bottom"';
                    lit qq(<div style="width:62px;height: ${len}px ;background:blue; border:1px solid red; writing-mode:tb-rl"></div>);
                end;
                css 'td', 'valign="bottom"';
                    lit qq(<div style="width:${i}px;height: ${len}px ;background:white;"></div>);
                end;
                ++$i;
            }
        end;
