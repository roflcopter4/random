...
class Hand(Queue):
    ...
    def __init__(self, cards, player_number, war_type):
        super().__init__(52)
    ...
    def get_card(self):
        # assert (not self.is_empty()), self.__player_number
        if self.get_size() <= 1:
            raise GameOver(self.__player_number)
        return self.dequeue()

    def get_war_cards(self):
        return [self.get_card() for _ in range(self.__war_type)]
...
def main(infile=None):
    ...
    while again:
        ...
        for i in (1, 2):
            hands[i] = Hand(hands[i], i, war_type)
        table = OnTable()
...
def play(hands, table):
    ...
    while not gameover:
        for i in (1, 2):
            faceup[i] = hands[i].get_card()
            table.place(i, faceup[i], True)
            print("Player %d: %-2d cards." % (i, (hands[i].get_size()+1)))

        print("CARDS ON TABLE")
        print(table.__str__())
        input()

        comp = FORTRAN_if(*[c2int(card) for card in faceup[1:]])
        if comp == 1:
            ...
            for i in (1, 2):
                for card in hands[i].get_war_cards():
                    table.place(i, card, False)
