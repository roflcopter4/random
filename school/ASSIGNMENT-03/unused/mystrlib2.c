...
void
my_strcpy(struct String *strto, char *strfrom)
{
        size_t len;
...
/* Concatenate two string literals to a struct String opject in addition to assigning
 * their total length to it. */
void
my_strcat(struct String *strto, char *str1, char *str2)
{
        size_t len1, len2;
        len1 = strlen(str1);
        len2 = strlen(str2) + 1;
...
void
my_strapp(struct String *str_orig, char *str_to_append)
{
...
void
my_strtcpy(struct String *strto, struct String *strfrom)
{
        if (strto->str != NULL)
                free(strto->str);
        if (strfrom->str == NULL)
...
void
my_strtcat(struct String *strto, struct String *str1, struct String *str2)
{
...
void
my_strtapp(struct String *str_orig, struct String *str_to_append)
{
        if (str_orig->str == NULL) {
...
void
SA_init(struct StringArray *str_array)
{
        int iter;

        if (str_array->arr == NULL) {
                my_xcalloc(str_array->arr, str_array->num, sizeof(struct String));
        } else {
...
void
SA_free(struct StringArray *str_array)
{
        int i;
...
struct StringArray
SA_append(struct StringArray *str_array, char *str)
{
...
struct StringArray
SA_append_string(struct StringArray *str_array, struct String str)
{
        int i;
        struct StringArray new_array = {NULL, str_array->num + 1};

        my_xcalloc(new_array.arr, new_array.num, sizeof(struct String));
        for (i = 0; i < str_array->num; ++i) {
...
void
get_stdin(struct StringArray *str_array)
{
        int x;
        struct String buf = {NULL, 0};
...
int
get_all_lines(struct StringArray *str_array, char *filename)
{
        int ch, it;
        FILE *fp;
