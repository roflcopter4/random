...
static struct linked_list * search(struct str_vec *taglist,
...
main(int argc, char **argv)
{
        ...
        struct str_vec *taglist = get_all_lines(tagfile);
        long i;
...
static void
get_colon_delim_data(char **data,
                     char *arg)
{
        int ch, it = 0, dit = 0;
        char buf[BUFSIZ];
       ...
       char **skip)
{
        ...
        for (uint32_t iter = 0; iter < taglist->num; ++iter)
        {
                ...
                if (!strchr(order, (int)data[0])) {
                        eprintf("Not in order: %s\n", data);
                        free(data);
                } else if (!is_correct_lang(ctov, lang, match_lang)) {
                        eprintf("Wrong lang: %s\n", data);
                        free(data);
                } else if (skip_tag(skip, data + 1)) {
                        eprintf("In skip list: %s\n", data);
                        free(data);
                } else if (ll_find_str(ll, data)) {
                        eprintf("Already in list: %s\n", data);
                        free(data);
                } else {
                        ll_add(ll, data);
                }
...
print_data(struct linked_list *ll, char *buffer)
{
        ...
        while (current != NULL) {
                char *ptr = strstr(buffer, (current->data + 1));
                if (ptr != NULL)
                        printf("%c\n%s\n", current->data[0], current->data + 1);
#ifdef DEBUG
                else
                        eprintf("Not in file: %c - %s\n", current->data[0], current->data + 1);
#endif
