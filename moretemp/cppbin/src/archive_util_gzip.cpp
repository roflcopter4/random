...
static void
get_size(struct archive_size *size, int fd, const char *name)
{
        off_t bytes_out = -1L;
        off_t bytes_in;

        bytes_in = lseek(fd, static_cast<off_t>(-8), SEEK_END);

        if (bytes_in != -1L) {
                uint8_t buf[8];
                bytes_in += 8L;

                const auto tmp = read(fd, reinterpret_cast<char *>(buf), sizeof(buf));
        ...
        size->archive      = static_cast<size_t>(bytes_in);
        size->uncompressed = static_cast<size_t>(bytes_out);
}
