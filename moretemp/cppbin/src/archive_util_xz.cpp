...
parse_indexes(struct xz_file *file, xz_file_info *xfi)
{
        ...
        lzma_ret ret = lzma_file_info_decoder(strm, &idx, MEM_LIMIT,
                                              static_cast<uint64_t>(file->st.st_size));
        if (ret != LZMA_OK)
                return false;
        ...
        for (bool done = false; !done ; )
        {
                ...
                case LZMA_SEEK_NEEDED:
                        ...
                        assert(strm->seek_pos <= static_cast<uint64_t>(file->st.st_size));
                        if (io_seek_src(file, static_cast<off_t>(strm->seek_pos))) {
                                warnx("%d, %s: %s\n", ret, file->name,
                                      message_strm(ret));
                                return false;
                        }
...
io_read(struct xz_file *file, io_buf *buf_union, size_t size)
{
        ...
        while (left > 0) {
                ...
                buf  += static_cast<size_t>(amount);
                left -= static_cast<size_t>(amount);
        }
...
message_strm(lzma_ret code)
{
        ...
        default: /* NOTREACHED */    return "Internal error (bug)";
        }
}
...
#define XZ_FILE_INFO_INIT { nullptr, 0, 0, true, 50000002 }
...
xz_size(struct archive_size *size, const char *filename)
{
        ...
        if (parse_indexes(file, &xfi)) {
                ...
                lzma_index_end(xfi.idx, nullptr);
                safe_close(file->fd);
        } else {
                safe_close(file->fd);
